...
const bloom   = $('bloom');
const replay  = $('replay');
const twistHub = $('twistHub');
const twistView = $('twistView');
const twistClose = $('twistClose');
const twistSub = $('twistSub');
const twistButtons = [...document.querySelectorAll('[data-twist]')];
const soniPhotos = ['75252.jpg','75249.jpg','75243.jpg','75246.jpg','75240.jpg','75234.jpg','75222.jpg'];
...
function treeFrame(now){
  ...
  if (!replayArmed && t >= T.done + 1.0){ replayArmed = true; armReplay(); setTimeout(openTwistHub, 700); }
...
/* ============================================================
   SONI TWIST SYSTEM — three interactive surprises after Act 4
   ============================================================ */
function openTwistHub(){
  if (!twistHub) return;
  twistHub.classList.add('is-open');
  twistHub.setAttribute('aria-hidden','false');
  twistView.classList.remove('is-visible');
  twistView.innerHTML='';
  twistClose.style.display='none';
  twistSub.textContent='Choose a button. Every button unlocks a different little surprise for Soni.';
}
function closeTwistView(){
  twistView.classList.remove('is-visible');
  twistView.innerHTML='';
  twistClose.style.display='none';
  twistSub.textContent='Choose a button. Every button unlocks a different little surprise for Soni.';
...
function showMemoryVault(){
  twistSub.textContent='A tiny gallery from the Birthday System archives — every frame says Soni.';
  twistView.innerHTML='<div class="memoryGrid">'+soniPhotos.map((p,i)=>`<figure class="memoryCard"><img src="./${p}" alt="Soni memory ${i+1}" loading="lazy"></figure>`).join('')+'</div>';
  twistView.classList.add('is-visible'); twistClose.style.display='block';
}
function showFutureMessage(){
  twistSub.textContent='System message received from the future…';
  twistView.innerHTML='<div class="twistView__message"><strong>Dear future Soni…</strong><span class="futureLine" id="futureLine"></span><span class="cursor"></span></div>';
  twistView.classList.add('is-visible'); twistClose.style.display='block';
  const text='I hope you never forget how beautifully you light up ordinary days. Keep smiling, keep dreaming, and keep being exactly you. Your best chapters are still loading… ✨';
  const el=$('futureLine'); let i=0;
  const tick=()=>{ if(!el || i>=text.length) return; el.textContent+=text[i++]; setTimeout(tick,32); }; tick();
}
function showSecret(){
  twistSub.textContent='SECRET LEVEL UNLOCKED';
  twistView.innerHTML='<div class="secretBox"><div class="secretBox__spark">✨</div><h3>Soni, you are the plot twist.</h3><p>The best part of this birthday story was never the animation.<br>It was having someone like <b>you</b> worth making it for. ❤️</p></div>';
  twistView.classList.add('is-visible'); twistClose.style.display='block';
  burstConfetti();
}
function burstConfetti(){
  for(let i=0;i<42;i++){
    const el=document.createElement('span'); el.textContent=['✦','♥','•','✧'][i%4];
    el.style.cssText=`position:fixed;left:${50+rand(-8,8)}%;top:50%;z-index:80;color:${['#ffd1df','#ff9fbd','#ffd36a','#fff'][i%4]};font-size:${rand(12,25)}px;pointer-events:none;`;
    document.body.appendChild(el);
    gsap.to(el,{x:rand(-45,45)+'vw',y:rand(-55,55)+'vh',rotation:rand(-360,360),opacity:0,duration:rand(1.3,2.2),ease:'power3.out',onComplete:()=>el.remove()});
  }
}
if(twistButtons.length){
  twistButtons.forEach(btn=>btn.addEventListener('click',()=>{
    const type=btn.dataset.twist;
    if(type==='memories') showMemoryVault();
    if(type==='future') showFutureMessage();
    if(type==='secret') showSecret();
  }));
  twistClose.addEventListener('click',closeTwistView);
}

/* back to Act 1, ready to be drawn again */
function resetAll(){
  if (twistHub){ twistHub.classList.remove('is-open'); twistHub.setAttribute('aria-hidden','true'); closeTwistView(); }
  treeStop();
  showWish(false);
