...
const SONI_IMAGES = [
  ...
  '75249.jpg',
  '75252.jpg'
].map((src, i) => ({ name: src, src: './' + src, index: i }));
...
// Always play the cinematic birthday film.
// Some Android phones have 'Remove animations' / reduced-motion enabled,
// which previously skipped directly to the final tree.
...
function armReplay(){
  ...
  if (soniTwists && !document.getElementById('openTwistsBtn')){
    const b = document.createElement('button');
    b.id = 'openTwistsBtn'; b.type = 'button'; b.className = 'replay';
    b.textContent = '✦ surprises for Soni'; b.style.right = '210px';
    b.addEventListener('click', showSoniTwists);
    document.querySelector('.scene').appendChild(b);
...
function resetAll(){
  treeStop();
  if (soniTwists) soniTwists.hidden = true;
  if (twistPanel) { twistPanel.hidden = true; twistPanel.innerHTML = ''; }
  const openBtn = document.getElementById('openTwistsBtn');
...
/* ============================================================
   SONI POST-FILM TWISTS — separate from the original four-act film
   ============================================================ */
const soniTwists = $('soniTwists');
const twistPanel = $('twistPanel');
const twistButtons = document.querySelectorAll('.twist-btn');

function openTwist(kind){
  if (!soniTwists || !twistPanel) return;
  soniTwists.hidden = false;
  twistPanel.hidden = false;
  if (kind === 'memories'){
    twistPanel.innerHTML = `<div class="soni-gallery">${SONI_IMAGES.map((im,i)=>`<figure class="soni-photo"><img src="${im.src}" alt="Soni memory ${i+1}" loading="lazy" draggable="false"></figure>`).join('')}</div>`;
  } else if (kind === 'future'){
    twistPanel.innerHTML = `<div class="twist-message">A little message from the future…<strong>Soni, I hope you never forget how beautifully you light up the lives around you. ❤️</strong></div>`;
  } else {
    twistPanel.innerHTML = `<div class="twist-secret"><span class="secret-heart">💗</span>The biggest twist was never the arrow, the roses, or the tree.<br><strong>Soni, you were the reason this whole little film had to exist.</strong></div>`;
  }
}

twistButtons.forEach(btn => btn.addEventListener('click', () => openTwist(btn.dataset.twist)));

function showSoniTwists(){
  if (!soniTwists) return;
  soniTwists.hidden = false;
}
